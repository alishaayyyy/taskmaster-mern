...
export async function signup(req,res){
  ...
  if(!name || !email || !password){
    return res.status(400).json({success: false, message :"kindly fill all your fields"});
  }
...
return res.status(400).json({success: false, message:"Valid name and email required"})
    ...
    try {
      ...
      if(exists){
       return  res.status(409).json({success: false, message:"Email is already exists by another account"})
      }
...
return res.status(400).json({success: false, message:"Password Invalid or less than 8 didgits"});
    }
    ...
    try {
      ...
      await user.save();
      return  res.json({success: true, message:"Password changes"});
    } catch (error) {
      return  res.status(500).json({success: false, message:"Server error"})
    }
  }